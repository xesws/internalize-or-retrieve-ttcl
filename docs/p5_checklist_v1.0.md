...
变更记录: v1.1 (2026-08-22) — 口径 3 定稿句; 追加口径 5–8; pressure-off 入交付清单。v1.2 (2026-08-22, 写作期裁决) — 追加"主叙事双主张结构"、C3 收缩表述、S7 纯描述句、残片披露位置、pod 处置时间线。

## 主叙事 (v1.2, JQ 定稿)

**双主张结构**, 骨架保持系统 proposal:
- **主张一 条件收益**: 压力下 router 追平 oracle (方差内, 口径 1)、碾压两个极端臂与双写臂。
- **主张二 生命周期信号**: 三证合一——pressure-off 无压力消融 (S1 recall 反超, 内化 recall 优势消失)、S6 否定结果 (逐条效用回归塌缩为 all-RAG, 信号不在 QA 轴)、S7 全局压垮 (双通道共存两败俱伤); 放置决策的价值住在生命周期轴上, 引出 sequence-aware 放置学习 future work。
